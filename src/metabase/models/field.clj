...
(defn- hierarchy-keyword-in [column-name & ancestor-types]
  (fn [k]
...
(defn- hierarchy-keyword-out [column-name & [fallback-type :as ancestor-types]]
  (fn [s]
    (when (seq s)
      (let [k (keyword s)]
        (if (some
...
(models/add-type! ::base-type
  :in  (hierarchy-keyword-in  :base_type :type/*)
  :out (hierarchy-keyword-out :base_type :type/*))

(models/add-type! ::effective-type
  :in  (hierarchy-keyword-in  :effective_type :type/*)
  :out (hierarchy-keyword-out :effective_type :type/*))

(models/add-type! ::semantic-type
  :in  (hierarchy-keyword-in  :semantic_type :Semantic/* :Relation/*)
  :out (hierarchy-keyword-out :semantic_type :Semantic/* :Relation/*))

(models/add-type! ::coercion-strategy
  :in  (hierarchy-keyword-in  :coercion_strategy :Coercion/*)
  :out (hierarchy-keyword-out :coercion_strategy :Coercion/*))
